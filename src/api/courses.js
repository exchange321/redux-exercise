...
const CourseList = {
    CSS: [
        ...
        },
        {
            ...
            id: 'css-transitions-and-transform',
        },
        {
    ...
    JAVASCRIPT: [
        ...
        },
        {
            ...
            id: 'javascript-loops-arrays-and-objects',
        },
        {
            title: 'jQuery Basics',
        ...
        },
        {
            ...
            id: 'interactive-web-pages-with-javascript',
        },
        {
...
class CourseAPI {
    static getAllCourses(topic) {
        return CourseList[topic];
    }

    static addNewTopic(topicName) {
        CourseList[topicName] = [];
    }

    static saveCourse(pointer, course) {
        const errMsg = {
            error: false,
            msg: '',
        };
        ...
        const { title, description, img_src } = course;
        ...
        if (!errMsg.error) {
            if (course.id) {
                const exitingCourseIndex = CourseList[pointer].findIndex(a => a.id === course.id);
                CourseList[pointer].splice(exitingCourseIndex, 1, course);
            } else {
                const newCourse = { ...course };
                newCourse.id = generateId(newCourse);
                CourseList[pointer].push(newCourse);
            }
        }
        return errMsg;
    }

    static deletePointer(pointer) {
        delete CourseList[pointer];
    }

    static deleteCourse(pointer, courseId) {
        const errMsg = {
            error: false,
            msg: '',
        };
        const indexOfCourseToDelete = CourseList[pointer].findIndex(course => (
            course.id === courseId
        ));
        CourseList[pointer].splice(indexOfCourseToDelete, 1);
        errMsg.msg = 'pass';
        return errMsg;
    }
}
